...
| `video_agent.generate_video_assets` | description + product + strategy + content (+ image) → 2 rendered videos | The 2 distinct prompts are generated for the product (`prompt_agent.generate_video_prompts`) and each generated video variant is mapped back to the calendar's video days; rendered via `models/wangp_client.py` with progress callbacks |
